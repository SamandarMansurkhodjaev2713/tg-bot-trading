...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    s["wma_20"] = ta.trend.wma_indicator(s["close"], 20)
    s["dema_20"] = ta.trend.dema_indicator(s["close"], 20)
    s["tema_20"] = ta.trend.triple_exponential_moving_average(s["close"], 20)
    s["kama_20"] = ta.momentum.kama(s["close"], 20)
    s["rsi_14"] = ta.momentum.rsi(s["close"], 14)
    s["stoch_k"] = ta.momentum.stoch(s["high"], s["low"], s["close"]).stoch
    s["stoch_d"] = ta.momentum.stoch_signal(s["high"], s["low"], s["close"]).stoch_signal
    s["stoch_rsi"] = ta.momentum.stochrsi(s["close"]).stochrsi
    s["cci_20"] = ta.trend.cci(s["high"], s["low"], s["close"], 20)
    s["roc_10"] = ta.momentum.roc(s["close"], 10)
    s["williams_r"] = ta.momentum.williams_r(s["high"], s["low"], s["close"], lbp=14)
    s["trix_15"] = ta.trend.trix(s["close"], 15)
    s["dpo_20"] = ta.trend.dpo_indicator(s["close"], 20)
    s["ppo"] = ta.momentum.ppo(s["close"]).ppo
    s["pvo"] = ta.momentum.pvo(s["close"], s["volume"]).pvo
    ...
    s["atr_14"] = ta.volatility.average_true_range(s["high"], s["low"], s["close"], 14)
    s["vol_hist_20"] = s["close"].pct_change().rolling(20).std() * np.sqrt(252)
    s["ulcer_14"] = ta.volatility.ulcer_index(s["close"], 14)
    s["chaikin_vol"] = ta.volatility.chaikin_volatility(s["high"], s["low"]) 
    s["adx_14"] = ta.trend.adx(s["high"], s["low"], s["close"], 14)
    s["di_pos"] = ta.trend.adx_pos(s["high"], s["low"], s["close"], 14)
    s["di_neg"] = ta.trend.adx_neg(s["high"], s["low"], s["close"], 14)
    s["psar"] = ta.trend.psar_up(s["high"], s["low"], s["close"]).fillna(method="ffill")
    s["supertrend"] = ta.trend.stc(s["close"]).stc
    ich = ta.trend.IchimokuIndicator(s["high"], s["low"]) 
    s["ich_a"] = ich.ichimoku_a()
    s["ich_b"] = ich.ichimoku_b()
    ...
    s["morning_star"] = ((s["close"] > s["open"]) & (s["open"].shift(1) > s["close"].shift(1)) & (s["close"].shift(2) < s["open"].shift(2))).astype(int)
    s["evening_star"] = ((s["close"] < s["open"]) & (s["open"].shift(1) < s["close"].shift(1)) & (s["close"].shift(2) > s["open"].shift(2))).astype(int)
    s["harami"] = ((s["open"] > s["close"].shift(1)) & (s["close"] < s["open"].shift(1))).astype(int)
    s["rsi_div"] = (s["rsi_14"].diff() * s["close"].diff() < 0).astype(int)
    s["trend_mode"] = ((s["adx_14"] > 25) & (s["di_pos"] != s["di_neg"]).astype(int))
    s["flat_mode"] = (s["adx_14"] < 20).astype(int)
